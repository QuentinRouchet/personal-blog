...
<x-app-layout>
    ...
    <div class="flex justify-center">
        <form action="{{ route('posts.update', $post) }}" method="post" enctype="multipart/form-data" class="block p-6 rounded-lg shadow-lg bg-white max-w-md">

            @method('put')
            @csrf

            <x-input-label for="title" value="post title" />
            <x-text-input id="title" name="title" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" placeholder="Title here" value="{{ $post->title }}" />
